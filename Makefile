...
CFLAGS	+=	$(INCLUDE) -D__SWITCH__ -DSTATUS_STRING="\"ftpd v$(APP_VERSION)\"" -DVERSION_STRING="\"$(APP_VERSION)\""
...
clean:
	@echo clean ...
	@rm -fr $(BUILD) $(OUTDIR) $(OUTPUT).pfs0 $(OUTPUT).nacp $(OUTPUT).elf $(OUTPUT).nso $(OUTPUT).nro
